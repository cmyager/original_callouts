...
class Settings(commands.Cog):
    ...
    async def seteventrole(self, ctx, *, event_role):
        """Set the lowest role that is able to create events
    ...
    async def seteventdeleterole(self, ctx, *, event_role):
        """Set the lowest role that is able to delete events
    ...
    async def seteventdeleterole_error(self, ctx, error):
        if isinstance(error, commands.MissingRequiredArgument):
            manager = MessageManager(ctx)
            #TODO
            event_role = get_event_delete_role(ctx, None)
    ...
    async def togglecleanup(self, ctx):
        """
        Toggle command message cleanup
    ...
    async def restart(self, ctx):
        """Restart the bot"""
        manager = MessageManager(ctx)
        _ = await manager.clean_messages()
        _ = await manager.send_message(f"Restart requested by *{ctx.message.author.display_name}*")
        sys.exit(1)
