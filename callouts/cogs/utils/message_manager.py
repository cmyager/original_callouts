...
class MessageManager:
    ...
    async def clean_messages(self):
        ...
        def message_needs_cleaning(message):
            if message is not None and message.id in [m.id for m in self.messages_to_clean]:
                return True
    ...
    async def send_private_message(self, message_text, attach_file=None):
        """Send a private message to the user"""
        return await self.ctx.author.send("{}".format(message_text), file=attach_file)
